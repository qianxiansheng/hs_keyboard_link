...
void iap_transfer(const HIDDevice& dev, uint8_t* data, uint32_t size, uint16_t packIdxReverse, bool isFirst)
{
...
void iap_business(const HIDDevice& dev, uint8_t* data, uint32_t size)
{
	assert(size >= 0);
...
uint16_t iap_transfer_read(const HIDDevice& dev, uint8_t* data, bool* isFirst, int timeout)
{
	uint8_t* buf = hid_report_buf + 1;
	uint16_t len = 0;
	char sbuf[64] = { 0 };
...
uint16_t iap_business_read(const HIDDevice& dev, uint8_t* data, size_t size, int timeout)
{
	bool isFirst = false;
...
void iap_run(IAPContext& ctx)
{
	...
	while (transfer_ongoing && iap_handle(ctx));
	transfer_ongoing = false;
}
...
void iap_run_switch_boot(IAPContext& ctx)
{
	transfer_ongoing = true;
	std::lock_guard<std::mutex> guard(transferMutex);
	while (transfer_ongoing && iap_handle_switch_boot(ctx));
	transfer_ongoing = false;
}
