...
void DrawSurfaceTri(glm::vec3 p1, glm::vec3 p2, glm::vec3 p3, std::vector<float>& output)
{
    glm::vec3 d1 = p1 - p2;
    glm::vec3 d2 = p3 - p2;
...
void DrawSurfaceRect(glm::vec3 p1, glm::vec3 p2, glm::vec3 p3, glm::vec3 p4, std::vector<float>& output)
{
    DrawSurfaceTri(p1, p2, p3, output);
    DrawSurfaceTri(p3, p4, p1, output);
}

void DrawKeycap(glm::vec3 pos, glm::vec3 shap, float padding, float thickness, std::vector<float>& output)
{
    glm::vec3 p[16];
    ...
    DrawSurfaceRect(p[0], p[4], p[7], p[3], output);
    DrawSurfaceRect(p[1], p[5], p[4], p[0], output);
    DrawSurfaceRect(p[2], p[6], p[5], p[1], output);
    DrawSurfaceRect(p[3], p[7], p[6], p[2], output);

    DrawSurfaceRect(p[8], p[0], p[3], p[11], output);
    DrawSurfaceRect(p[9], p[1], p[0], p[8], output);
    DrawSurfaceRect(p[10], p[2], p[1], p[9], output);
    DrawSurfaceRect(p[11], p[3], p[2], p[10], output);

    DrawSurfaceRect(p[15], p[7], p[4], p[12], output);
    DrawSurfaceRect(p[12], p[4], p[5], p[13], output);
    DrawSurfaceRect(p[13], p[5], p[6], p[14], output);
    DrawSurfaceRect(p[14], p[6], p[7], p[15], output);

    DrawSurfaceRect(p[11], p[10], p[9], p[8], output);
    DrawSurfaceRect(p[12], p[13], p[14], p[15], output);
}

#define SECTOR 180.0f

std::vector<float> RenderModel()
{
    std::vector<float> collection(0);

    glm::vec3 global_pos(0.0f, 0.0f, 0.0f);

    //DrawKeycap(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(1.0f, 1.0f, 1.0f), 0.2f, 0.1f, collection);
    ...
    for (int i = 0; i < keyBtnViewSize; ++i)
    {
        ...
        DrawKeycap(pos1, shape, 0.1f, 0.1f, collection);
    }
